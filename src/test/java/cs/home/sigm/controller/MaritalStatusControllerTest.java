...
@SpringBootTest
public class MaritalStatusControllerTest extends SIGMApplicationTestSetup {
	...
	public void shouldLoadAllMaritalStatusForPtBR() throws Exception {
		...
		findAll("pt_BR").andExpect(status().isOk()).andExpect(jsonPath("$", hasSize(6)))
	...
	public void shouldLoadAllMaritalStatusForEnUS() throws Exception {
		...
		findAll("en_US").andExpect(status().isOk()).andExpect(jsonPath("$", hasSize(6)))
	...
	public void shouldLoadAllMaritalStatusForEsES() throws Exception {
		...
		findAll("es_ES").andExpect(status().isOk()).andExpect(jsonPath("$", hasSize(6)))
	...
	public void shouldGetMarried() throws Exception {
		...
		findSingle("en_US", 3L).andExpect(status().isOk()).andExpect(jsonPath("$.language", is("en_US")))
				.andExpect(jsonPath("$.title", is("Married")));
	}
