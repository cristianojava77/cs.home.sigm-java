...
public class RelationshipControllerTest {
	...
	public void shouldLoadAllRelationshipsForPtBR() throws Exception {
		...
		findAll("pt_BR").andExpect(status().isOk()).andExpect(jsonPath("$", hasSize(5))).andExpect(jsonPath("$[0].language", is("pt_BR")));
	}

	@Test
	public void shouldLoadAllRelationshipsForEnUS() throws Exception {
		log.info("|:: TEST ::| >>> Should load all Relationships for en_US.");
		findAll("en_US").andExpect(status().isOk()).andExpect(jsonPath("$", hasSize(5))).andExpect(jsonPath("$[0].language", is("en_US")));
	}

	@Test
	public void shouldLoadAllRelationshipsForEsES() throws Exception {
		log.info("|:: TEST ::| >>> Should load all Relationships for es_ES.");
		findAll("es_ES").andExpect(status().isOk()).andExpect(jsonPath("$", hasSize(5))).andExpect(jsonPath("$[0].language", is("es_ES")));
	}

	@Test
	public void shouldGetSpouse() throws Exception {
		log.info("|:: TEST ::| >>> Should get Brazil.");
		findSingle("en_US", 1L).andExpect(status().isOk()).andExpect(jsonPath("$.language", is("en_US"))).andExpect(jsonPath("$.title", is("Spouse")));
	}

	@Test
	public void shouldGetMother() throws Exception {
		log.info("|:: TEST ::| >>> Should get Argentina.");
		findSingle("es_ES", 3L).andExpect(status().isOk()).andExpect(jsonPath("$.language", is("es_ES"))).andExpect(jsonPath("$.title", is("Madre")));
	}
